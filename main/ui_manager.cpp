...
const char* UIManager::getCharset() { return WIFI_CHARSET; }
int UIManager::getCharsetLength() { return strlen(WIFI_CHARSET); }
...
const char* menu_item_labels_page_1[MENU_PAGE1_ITEM_COUNT] = {
  "Max Temp Lock", "Heater Calibration", "IR Emissivity", "TC Probe Cal", "Next Page >"
};

const char* menu_item_labels_page_2[MENU_PAGE2_ITEM_COUNT] = {
  "Turn off When Idle", "Startup Mode", "Sound", "Temp Unit", "About", "Next Page >"
};
...
void UIManager::switchToAutoMode() {
    _previous_screen = SCREEN_AUTO_MODE; _current_screen = SCREEN_AUTO_MODE; resetInactivityTimer();
}
void UIManager::switchToManualMode() {
    _previous_screen = SCREEN_MANUAL_MODE; _current_screen = SCREEN_MANUAL_MODE; _manual_selection = _manual_confirmed_preset; resetInactivityTimer();
}
void UIManager::switchToStandby() {
    _previous_screen = SCREEN_STANDBY; _current_screen = SCREEN_STANDBY; resetInactivityTimer();
}
...
void UIManager::enterQuickEdit() {
    if (_standby_selection >= 0 && _standby_selection <= 2) {
        _current_screen = SCREEN_QUICK_EDIT; _quick_edit_step = Q_EDIT_TARGET; _menu_step_accumulator = 0.0f; resetInactivityTimer();
    }
}
void UIManager::enterQuickEditAuto() {
    if (_auto_selection >= 0 && _auto_selection <= 2) {
        _current_screen = SCREEN_QUICK_EDIT_AUTO; _quick_edit_step = Q_EDIT_TARGET; _menu_step_accumulator = 0.0f; resetInactivityTimer();
    }
}
void UIManager::enterQuickEditManual() {
    if (_manual_selection >= 0 && _manual_selection <= 3) {
        _current_screen = SCREEN_QUICK_EDIT_MANUAL; _quick_edit_step = Q_EDIT_TARGET; _menu_step_accumulator = 0.0f; resetInactivityTimer();
    }
}
void UIManager::resetInactivityTimer() { _last_activity_time = millis(); }
...
bool UIManager::checkInactivity(ConfigState& config, bool& has_go_to, float& go_to) { 
  ...
  if (elapsed > sleep_timeout) {
    _current_screen = SCREEN_SLEEP; has_go_to = false; go_to = NAN; return true;
  }
  return false;
}
...
void UIManager::draw(const AppState& state, const ConfigState& config) {
  ...
  // Warning Logic
  _show_warning = false;
  for (int i = 0; i < 3; i++) { if (state.heater_cutoff_state[i]) { _show_warning = true; break; } }
  if (!_show_warning) { for (int i = 0; i < 2; i++) { if (!isnan(state.ir_ambient[i]) && state.ir_ambient[i] > 80.0f) { _show_warning = true; break; } } }
  ...
  switch (_current_screen) {
    case SCREEN_SLEEP:
      ...
      _spr.unloadFont(); _spr.pushSprite(0, 0); return; 
...
void UIManager::drawTaskBar() {
    int w = _spr.width(); int h = 24;
    _spr.fillRect(0, 0, w, h, C_BLACK); _spr.drawFastHLine(0, h-1, w, TFT_DARKGREY);
    ...
    switch (wifi_status) {
        case WIFI_STATUS_CONNECTED:
            if (rssi > -50) { wifi_icon = wifi_connected; wifi_color = TFT_GREEN; } 
            else if (rssi > -70) { wifi_icon = wifi_signal_3; wifi_color = TFT_GREEN; } 
            else if (rssi > -80) { wifi_icon = wifi_signal_2; wifi_color = TFT_YELLOW; } 
            else { wifi_icon = wifi_signal_1; wifi_color = TFT_ORANGE; }
            break;
        case WIFI_STATUS_CONNECTING: wifi_icon = _blink_state ? wifi_connecting_1 : wifi_connecting_2; wifi_color = TFT_CYAN; break;
        default: wifi_icon = wifi_disconnected; wifi_color = TFT_RED; break;
    }
...
void UIManager::drawHeader(const char* title) {
    ...
    _spr.setTextColor(TFT_BLACK, TFT_WHITE); _spr.setTextDatum(MC_DATUM); _spr.loadFont(Arial18);
    _spr.drawString(title, _spr.width() / 2, y_header + (h_header/2)); _spr.unloadFont();
}
...
void UIManager::drawSettingsPage1(const AppState& state, const ConfigState& config) {
  ...
  for (int i = 0; i < MENU_PAGE1_ITEM_COUNT; ++i) {
    ...
    _spr.setTextColor(text_color, bg_color); _spr.setTextDatum(MC_DATUM); _spr.loadFont(Arial18);
...
void UIManager::drawSettingsWiFiMenu(const AppState& state, const ConfigState& config) {
  ...
  for (int i = 0; i < WIFI_MENU_ITEM_COUNT; ++i) {
    ...
    _spr.setTextColor(text_color, bg_color); _spr.setTextDatum(ML_DATUM); _spr.loadFont(Arial18);
    char buf[50];
    switch (i) {
      case WIFI_MENU_SSID:
        ...
        _spr.drawString(buf, X_MARGIN + 5, y + ITEM_HEIGHT / 2); break;
      case WIFI_MENU_PASSWORD:
        snprintf(buf, 50, "Password: %s", (strlen(config.wifi_config.password) > 0) ? "****" : "(not set)");
        _spr.drawString(buf, X_MARGIN + 5, y + ITEM_HEIGHT / 2); break;
      default:
        _spr.setTextDatum(MC_DATUM); _spr.drawString(wifi_menu_labels[i], _spr.width() / 2, y + ITEM_HEIGHT / 2); break;
    }
    _spr.unloadFont();
  }
  _spr.setTextColor(TFT_DARKGREY, TFT_BLACK); _spr.setTextDatum(BC_DATUM); _spr.setTextSize(1);
  WiFiConnectionStatus wifi_status = getWiFiStatus();
  const char* status_str = (wifi_status == WIFI_STATUS_CONNECTED) ? "Connected" : (wifi_status == WIFI_STATUS_CONNECTING) ? "Connecting..." : "Disconnected";
  char info_buf[60]; snprintf(info_buf, 60, "Status: %s", status_str);
...
void UIManager::drawSettingsWiFiStatus(const AppState& state, const ConfigState& config) {
  ...
  _spr.loadFont(Arial18); _spr.setTextSize(1);
  int y = 70, line_h = 25, x_label = 10, x_value = 120;
  
  WiFiConnectionStatus wifi_status = getWiFiStatus();
  _spr.setTextDatum(ML_DATUM); _spr.setTextColor(TFT_WHITE, TFT_BLACK); _spr.drawString("Status:", x_label, y);
  uint16_t status_color = (wifi_status == WIFI_STATUS_CONNECTED) ? TFT_GREEN : (wifi_status == WIFI_STATUS_CONNECTING) ? TFT_CYAN : TFT_RED;
  ...
  y += line_h; _spr.setTextColor(TFT_WHITE, TFT_BLACK); _spr.drawString("SSID:", x_label, y);
  _spr.setTextColor(TFT_CYAN, TFT_BLACK); _spr.drawString((strlen(config.wifi_config.ssid) > 0) ? config.wifi_config.ssid : "(default)", x_value, y);
  
  y += line_h; _spr.setTextColor(TFT_WHITE, TFT_BLACK); _spr.drawString("Signal:", x_label, y);
  int rssi = getWiFiSignalStrength();
  char rssi_buf[20];

  if (wifi_status == WIFI_STATUS_CONNECTED) {
    snprintf(rssi_buf, 20, "%d dBm", rssi);
    _spr.setTextColor((rssi > -50) ? TFT_GREEN : (rssi > -70) ? TFT_YELLOW : TFT_ORANGE, TFT_BLACK);
  } else { snprintf(rssi_buf, 20, "N/A"); _spr.setTextColor(TFT_DARKGREY, TFT_BLACK); }
  ...
  y += line_h; _spr.setTextColor(TFT_WHITE, TFT_BLACK); _spr.drawString("Config:", x_label, y);
...
void UIManager::drawSettingsStartup(const AppState& state, const ConfigState& config) {
  ...
  for (int i = 0; i < STARTUP_MODE_COUNT; ++i) {
    ...
    if (i == _selected_menu_item) _spr.drawRect(X_MARGIN, y, w, ITEM_HEIGHT, TFT_DARKGREY);
    _spr.setTextColor(text_color, bg_color); _spr.setTextDatum(MC_DATUM); _spr.loadFont(Arial18);
    _spr.drawString(startup_mode_labels[i], _spr.width() / 2, y + ITEM_HEIGHT / 2); _spr.unloadFont();
  }
  _spr.setTextColor(TFT_YELLOW, TFT_BLACK); _spr.setTextDatum(BC_DATUM); _spr.setTextSize(1); _spr.drawString("Rot: Select | Press: Save", _spr.width() / 2, _spr.height() - 10);
}
...
void UIManager::drawSettingsIdleOff(const AppState& state, const ConfigState& config) {
  ...
  for (int i = 0; i < IDLE_OFF_ITEM_COUNT; ++i) {
    ...
    if (i == _selected_menu_item) _spr.drawRect(X_MARGIN, y, w, ITEM_HEIGHT, TFT_DARKGREY);
    _spr.setTextColor(text_color, bg_color); _spr.setTextDatum(MC_DATUM); _spr.loadFont(Arial18);
    _spr.drawString(idle_off_labels[i], _spr.width() / 2, y + ITEM_HEIGHT / 2); _spr.unloadFont();
  }
}
...
void UIManager::drawSettingsTempUnit(const AppState& state, const ConfigState& config) {
    ...
    for (int i = 0; i < 2; ++i) {
        ...
        if (i == _selected_menu_item) _spr.drawRect(X_MARGIN, y, w, ITEM_HEIGHT, TFT_DARKGREY);
        _spr.setTextColor(text_color, bg_color); _spr.setTextDatum(MC_DATUM); _spr.loadFont(Arial18);
        _spr.drawString(temp_unit_labels[i], _spr.width() / 2, y + ITEM_HEIGHT / 2); _spr.unloadFont();
    }
}

void UIManager::drawSettingsSound(const AppState& state, const ConfigState& config) { 
    drawHeader("Sound Settings");
    int w = _spr.width() - 60; int h = 40; int x = 30; int y = 90;
    _spr.fillRect(x, y, w, h, C_SELECT_BG); _spr.drawRect(x, y, w, h, TFT_DARKGREY);
...
void UIManager::drawSettingsEmissivity(const AppState& state, const ConfigState& config) {
  ...
  for(int i=0; i<2; i++) {
      ...
      _spr.setTextColor(txt_color, bg_color); _spr.setTextDatum(MC_DATUM);
      char buf[40];
      if (i==0) {
...
void UIManager::drawSettingsTCProbeCal(const AppState& state, const ConfigState& config) {
  ...
  _spr.setTextColor(TFT_WHITE, TFT_BLACK); _spr.setTextDatum(MC_DATUM); _spr.loadFont(Arial18);
  char buf[32]; int info_y = 80;
  
  if (isnan(state.tc_probe_temp)) {
    _spr.setTextColor(TFT_LIGHTGREY, TFT_BLACK); _spr.drawString("Raw: ---", _spr.width() / 2, info_y);
    _spr.drawString("Offset: ---", _spr.width() / 2, info_y + 25);
    _spr.setTextColor(TFT_DARKGREY, TFT_BLACK); _spr.drawString("Val: ---", _spr.width() / 2, info_y + 50);
  } else {
  ...
  int w = _spr.width() - 40; int h = 26;
  for (int i = 0; i < 2; i++) {
     int y = start_y + (i * 30); 
     uint16_t bg = (i == _selected_menu_item) ? C_SELECT_BG : TFT_BLACK;
     uint16_t txt = (i == _selected_menu_item) ? C_SELECT_TXT : TFT_WHITE;
     _spr.fillRect(20, y, w, h, bg); if(i == _selected_menu_item) _spr.drawRect(20, y, w, h, TFT_DARKGREY);
...
void UIManager::drawSettingsCalibrationSelect(const AppState& state, const ConfigState& config) {
  ...
  char item_labels[3][30]; int start_y = 75; int w = _spr.width() - 20; int h = 30;
  _spr.loadFont(Arial18);

  for (int i = 0; i < 3; ++i) {
    int y = start_y + i * 35; 
    uint16_t bg = TFT_BLACK; uint16_t txt = TFT_WHITE;
  ...
  _spr.unloadFont();
  _spr.setTextColor(TFT_YELLOW, TFT_BLACK); _spr.setTextDatum(BC_DATUM); _spr.setTextSize(1); _spr.drawString("Rot: Select/Edit | Press: Toggle", _spr.width() / 2, _spr.height() - 10);
}
...
void UIManager::drawSettingsHeaterCalibrate(const AppState& state) { drawHeader("Heater Cal (Unused)"); }
...
void UIManager::drawSettingsAbout(const AppState& state) {
    ...
    _spr.setTextColor(TFT_WHITE, TFT_BLACK); _spr.setTextDatum(MC_DATUM); _spr.loadFont(Arial18); _spr.setTextSize(1);
    int line_height = 12; int y_center = _spr.height() / 2 + 10; int y_start = y_center - (line_height * 3);
...
bool UIManager::handleButtonSingleClick(ConfigState& config, float& go_to, bool& has_go_to) {
  ...
  switch (_current_screen) {
    case SCREEN_STANDBY:
        ...
        } else if (_standby_selection == 5) { openSettings(); }
        return true;
    case SCREEN_QUICK_EDIT:
        if (_quick_edit_step == Q_EDIT_TARGET) _quick_edit_step = Q_EDIT_MAX;
        else { if (_save_callback) _save_callback(config); _current_screen = SCREEN_STANDBY; }
        return true;
    case SCREEN_QUICK_EDIT_AUTO:
        if (_quick_edit_step == Q_EDIT_TARGET) _quick_edit_step = Q_EDIT_MAX;
        else { if (_save_callback) _save_callback(config); _current_screen = SCREEN_AUTO_MODE; }
        return true;
    ...
    case SCREEN_QUICK_EDIT_MANUAL:
        if (_quick_edit_step == Q_EDIT_TARGET) _quick_edit_step = Q_EDIT_MAX; 
        else { if (_save_callback) _save_callback(config); _current_screen = SCREEN_MANUAL_MODE; }
        return true;
    case SCREEN_SETTINGS_PAGE_1: 
      switch (_selected_menu_item) {
        case MENU_PAGE1_MAX_TEMP_LOCK: _current_screen = SCREEN_SETTINGS_MAX_TEMP_LOCK; _temp_edit_value = config.max_temp_lock; break;
        case MENU_PAGE1_CALIBRATION: _current_screen = SCREEN_SETTINGS_CALIBRATION_SELECT; _selected_menu_item = 0; _is_editing_calibration = false; break;
        case MENU_PAGE1_EMISSIVITY: _current_screen = SCREEN_SETTINGS_EMISSIVITY; _selected_menu_item = 0; break;
        case MENU_PAGE1_TC_PROBE_CAL: _current_screen = SCREEN_SETTINGS_TC_PROBE_CAL; _selected_menu_item = 0; break;
        case MENU_PAGE1_NEXT_PAGE: _current_screen = SCREEN_SETTINGS_PAGE_2; _selected_menu_item_page_2 = 0; break;
      }
    ...
    case SCREEN_SETTINGS_PAGE_2: 
      switch (_selected_menu_item_page_2) {
        ...
        // [MODIFIED] Navigate to Page 3
        case MENU_PAGE2_NEXT_PAGE: _current_screen = SCREEN_SETTINGS_PAGE_3; _selected_menu_item_page_3 = 0; break;
      }
      _menu_step_accumulator = 0.0f; return true;
    ...
    case SCREEN_SETTINGS_WIFI_MENU:
      switch (_selected_wifi_menu_item) {
        ...
        case WIFI_MENU_CONNECT: if (_wifi_reconnect_callback) _wifi_reconnect_callback(); break;
  ...
  case SCREEN_SETTINGS_WIFI_PASSWORD:
    ...
    case SCREEN_SETTINGS_HEATER_TARGET_TEMP:
      ...
      if (_save_callback) _save_callback(config);
      _current_screen = SCREEN_SETTINGS_HEATER_MAX_TEMP; _temp_edit_value = config.max_temps[_selected_menu_item]; _menu_step_accumulator = 0.0f; return true;
    case SCREEN_SETTINGS_HEATER_MAX_TEMP:
      config.max_temps[_selected_menu_item] = _temp_edit_value;
      if (_save_callback) _save_callback(config);
      _current_screen = SCREEN_SETTINGS_PAGE_1; _menu_step_accumulator = 0.0f; return true;
    case SCREEN_SETTINGS_CALIBRATION_SELECT:
      _is_editing_calibration = !_is_editing_calibration;
      if (!_is_editing_calibration) { if (_save_callback) _save_callback(config); }
      _menu_step_accumulator = 0.0f; return true;
    ...
    case SCREEN_SETTINGS_IDLE_OFF:
      ...
      if (_save_callback) _save_callback(config);
      _current_screen = SCREEN_SETTINGS_PAGE_2; _menu_step_accumulator = 0.0f; return true;
    case SCREEN_SETTINGS_SOUND: 
      config.sound_on = !config.sound_on;
      if (_save_callback) _save_callback(config); return true;
    case SCREEN_SETTINGS_STARTUP: 
      config.startup_mode = (StartupMode)_selected_menu_item;
      if (_save_callback) _save_callback(config);
      _current_screen = SCREEN_SETTINGS_PAGE_2; _menu_step_accumulator = 0.0f; return true;
    case SCREEN_SETTINGS_EMISSIVITY:
      if (_selected_menu_item == 0) _selected_menu_item = 1; else _selected_menu_item = 0; return true;
    case SCREEN_SETTINGS_TEMP_UNIT:
      config.temp_unit = (_selected_menu_item == 0) ? 'C' : 'F';
      _current_screen = SCREEN_SETTINGS_PAGE_2; 
      if (_save_callback) _save_callback(config); _menu_step_accumulator = 0.0f; return true;
...
bool UIManager::handleButtonDoubleClick(ConfigState& config) {
  ...
  resetInactivityTimer();
  switch (_current_screen) {
    case SCREEN_SETTINGS_PAGE_1: case SCREEN_SETTINGS_PAGE_2: case SCREEN_SETTINGS_PAGE_3: _current_screen = SCREEN_STANDBY; break;
    ...
    case SCREEN_SETTINGS_WIFI_SSID:
      if (_char_entry_editing) { _char_entry_editing = false; } else {
...
bool UIManager::handleEncoderRotation(float steps, ConfigState& config) {
  ...
  switch (_current_screen) {
    case SCREEN_STANDBY: {
      int new_pos = _standby_selection + change; if (new_pos < 0) new_pos = 0; if (new_pos >= 3) new_pos = 2; 
      _standby_selection = new_pos; break;
    }
    case SCREEN_AUTO_MODE: {
      int new_pos = _auto_selection + change; if (new_pos < 0) new_pos = 0; if (new_pos > 2) new_pos = 2; _auto_selection = new_pos; break;
    }
    case SCREEN_MANUAL_MODE: {
      int new_pos = _manual_selection + change; if (new_pos < 0) new_pos = 0; if (new_pos > 3) new_pos = 3; _manual_selection = new_pos; break;
    }
    case SCREEN_QUICK_EDIT: {
    ...
    case SCREEN_SETTINGS_PAGE_1: { 
      int new_pos = _selected_menu_item + change; if (new_pos < 0) new_pos = 0; if (new_pos >= MENU_PAGE1_ITEM_COUNT) new_pos = MENU_PAGE1_ITEM_COUNT - 1; _selected_menu_item = new_pos; break;
    }
    case SCREEN_SETTINGS_PAGE_2: { 
      int new_pos = _selected_menu_item_page_2 + change; if (new_pos < 0) new_pos = 0; if (new_pos >= MENU_PAGE2_ITEM_COUNT) new_pos = MENU_PAGE2_ITEM_COUNT - 1; _selected_menu_item_page_2 = new_pos; break;
    }
    // [ADDED] Page 3 & WiFi Menu Navigation
    case SCREEN_SETTINGS_PAGE_3: {
      int new_pos = _selected_menu_item_page_3 + change; if (new_pos < 0) new_pos = 0; if (new_pos >= MENU_PAGE3_ITEM_COUNT) new_pos = MENU_PAGE3_ITEM_COUNT - 1; _selected_menu_item_page_3 = new_pos; break;
    }
    case SCREEN_SETTINGS_WIFI_MENU: {
      int new_pos = _selected_wifi_menu_item + change; if (new_pos < 0) new_pos = 0; if (new_pos >= WIFI_MENU_ITEM_COUNT) new_pos = WIFI_MENU_ITEM_COUNT - 1; _selected_wifi_menu_item = new_pos; break;
    }
    case SCREEN_SETTINGS_WIFI_SSID:
    case SCREEN_SETTINGS_WIFI_PASSWORD: {
      if (_char_entry_editing) {
    ...
    case SCREEN_SETTINGS_TC_PROBE_CAL: {
      int new_pos = _selected_menu_item + change; if (new_pos < 0) new_pos = 0; if (new_pos > 1) new_pos = 1; _selected_menu_item = new_pos; break;
    }
    case SCREEN_SETTINGS_CALIBRATION_SELECT: {
      if (_is_editing_calibration) {
          float val = config.tc_offsets[_selected_menu_item]; val += (float)change * 0.1f;
          if (val < -20.0f) val = -20.0f; if (val > 20.0f) val = 20.0f; config.tc_offsets[_selected_menu_item] = val;
      } else {
          int new_pos = _selected_menu_item + change; if (new_pos < 0) new_pos = 0; if (new_pos >= 3) new_pos = 2; _selected_menu_item = new_pos;
      }
      break;
    }
    case SCREEN_SETTINGS_HEATER_TARGET_TEMP: {
      _temp_edit_value += (float)change * 0.5f; if (_temp_edit_value < 0) _temp_edit_value = 0; if (_temp_edit_value > config.max_temps[_selected_menu_item]) _temp_edit_value = config.max_temps[_selected_menu_item]; break;
    }
    case SCREEN_SETTINGS_HEATER_MAX_TEMP: {
      _temp_edit_value += (float)change * 0.5f; if (_temp_edit_value < config.target_temps[_selected_menu_item]) _temp_edit_value = config.target_temps[_selected_menu_item]; if (_temp_edit_value > config.max_temp_lock) _temp_edit_value = config.max_temp_lock; break;
    }
    case SCREEN_SETTINGS_HEATER_CALIBRATE: { _temp_edit_value += (float)change * 0.1f; break; }
    case SCREEN_SETTINGS_MAX_TEMP_LOCK: {
      _temp_edit_value += (float)change * 0.5f; if (_temp_edit_value < 0) _temp_edit_value = 0; if (_temp_edit_value > 800) _temp_edit_value = 800; break;
    }
    case SCREEN_SETTINGS_EMISSIVITY: {
      float* target = &config.ir_emissivity[_selected_menu_item]; *target += (float)change * 0.01f; if (*target < 0.1f) *target = 0.1f; if (*target > 1.0f) *target = 1.0f; break;
    }
    case SCREEN_SETTINGS_STARTUP: { 
      int new_pos = _selected_menu_item + change; if (new_pos < 0) new_pos = 0; if (new_pos >= STARTUP_MODE_COUNT) new_pos = STARTUP_MODE_COUNT - 1; _selected_menu_item = new_pos; break;
    }
    case SCREEN_SETTINGS_IDLE_OFF: {
      int new_pos = _selected_menu_item + change; if (new_pos < 0) new_pos = 0; if (new_pos >= IDLE_OFF_ITEM_COUNT) new_pos = IDLE_OFF_ITEM_COUNT - 1; _selected_menu_item = new_pos; break;
    }
    case SCREEN_SETTINGS_SOUND: { if (change != 0) { config.sound_on = !config.sound_on; if (_save_callback) _save_callback(config); } break; }
    case SCREEN_SETTINGS_TEMP_UNIT: {
      int new_pos = _selected_menu_item + change; if (new_pos < 0) new_pos = 0; if (new_pos > 1) new_pos = 1; _selected_menu_item = new_pos; break;
    }
...
float UIManager::convertTemp(float temp_c, char unit) { if (unit == 'F' && !isnan(temp_c)) return temp_c * 1.8f + 32.0f; return temp_c; }
float UIManager::convertDelta(float temp_c, char unit) { if (unit == 'F') return temp_c * 1.8f; return temp_c; }
...
void UIManager::drawStandbyScreen(const AppState& state, const ConfigState& config) {
  ...
  for (int i = 0; i < 3; i++) {
      ...
      _spr.fillRect(x, y, heater_w, heater_h, bg_color); int center_x = x + (heater_w / 2);
      _spr.loadFont(Arial18); _spr.setTextColor((isLocked && _blink_state) ? C_RED_TXT : C_BLACK, bg_color);
      _spr.setTextDatum(TC_DATUM); char title[15]; snprintf(title, 15, "Heater %d", i+1); _spr.drawString(title, center_x, y + 8); 
      ...
      if (_current_screen == SCREEN_QUICK_EDIT && _standby_selection == i) {
          ...
          if (isLocked) { _spr.setTextColor(_blink_state ? C_RED_TXT : bg_color, bg_color); _spr.drawString("Lock", center_x, status_y + val_offset); } 
          else if (isControlledByOther) { _spr.setTextColor(C_GREY_TXT, bg_color); _spr.drawString("In-use", center_x, status_y + val_offset); }
          else if (!isActive) { _spr.setTextColor(C_GREY_TXT, bg_color); _spr.drawString("OFF", center_x, status_y + val_offset); }
          else {
              bool isRunning = false; if (globalRun) { if (state.manual_was_started) isRunning = true; }
              if (isRunning) { if (state.heater_ready[i]) { _spr.setTextColor(C_GREEN_TXT, bg_color); _spr.drawString("Ready", center_x, status_y + val_offset); } else { _spr.setTextColor(TFT_ORANGE, bg_color); _spr.drawString("Heating", center_x, status_y + val_offset); } } 
              else { _spr.setTextColor(TFT_BLUE, bg_color); _spr.drawString("Standby", center_x, status_y + val_offset); }
          }
      } 
      _spr.unloadFont();
      if (_standby_selection == i) { _spr.drawRect(x, y, heater_w, heater_h, TFT_RED); _spr.drawRect(x+1, y+1, heater_w-2, heater_h-2, TFT_RED); } 
      else { _spr.drawRect(x, y, heater_w, heater_h, C_BLACK); }
  }
...
void UIManager::drawAutoModeScreen(const AppState& state, const ConfigState& config) {
    ...
    for (int i = 0; i < 3; i++) { 
        ...
        if (isSelected) { _spr.drawRect(x, cycle_start_y, cycle_box_w, cycle_box_h, TFT_RED); _spr.drawRect(x+1, cycle_start_y+1, cycle_box_w-2, cycle_box_h-2, TFT_RED); } 
        else { _spr.drawRect(x, cycle_start_y, cycle_box_w, cycle_box_h, TFT_BLACK); }

        _spr.loadFont(Arial18); _spr.setTextColor(C_BLACK, bg_color); _spr.setTextDatum(TC_DATUM); snprintf(buf, 20, "Cycle %d", i+1); _spr.drawString(buf, x + (cycle_box_w/2), cycle_start_y + 8); 
        ...
        _spr.loadFont(Arial12); _spr.setTextColor(C_BLACK, bg_color); _spr.drawString("SET", x + (cycle_box_w/2), set_y_label);
        _spr.loadFont(Arial18);
        ...
        if (isEditingThis) {
            ...
            else if (isLocked) { _spr.setTextColor(_blink_state ? C_RED_TXT : bg_color, bg_color); _spr.drawString("Lock", x + (cycle_box_w/2), status_y_val); } 
            else if (!isThisCycleActive || !globalRun) { _spr.setTextColor(TFT_BLUE, bg_color); _spr.drawString("Standby", x + (cycle_box_w/2), status_y_val); } 
            else {
                if (isReady) { _spr.setTextColor(C_GREEN_TXT, bg_color); _spr.drawString("Ready", x + (cycle_box_w/2), status_y_val); } 
                else { int badge_w = 80, badge_h = 26, center_x = x + (cycle_box_w / 2); _spr.fillRect(center_x - (badge_w / 2), status_y_val, badge_w, badge_h, TFT_WHITE); _spr.setTextDatum(MC_DATUM); _spr.setTextColor(TFT_ORANGE, bg_color); _spr.drawString("Heating", center_x, status_y_val + (badge_h / 2)); _spr.setTextDatum(TC_DATUM); }
            }
        }
        _spr.unloadFont();
    }
...
void UIManager::drawManualModeScreen(const AppState& state, const ConfigState& config) {
    ...
    for (int i = 0; i < 4; i++) { 
        ...
        if (isCursor) { _spr.drawRect(x, y, preset_box_w, preset_box_h, TFT_RED); _spr.drawRect(x+1, y+1, preset_box_w-2, preset_box_h-2, TFT_RED); } else { _spr.drawRect(x, y, preset_box_w, preset_box_h, TFT_BLACK); }

        int split_w = preset_box_w * 0.30; 
        _spr.loadFont(Arial18); _spr.setTextColor(C_BLACK, bg_color); _spr.setTextDatum(MC_DATUM); snprintf(buf, 20, "P%d", i+1); _spr.drawString(buf, x + (split_w / 2), y + (preset_box_h / 2));
        ...
        } else { _spr.setTextColor(C_BLACK, bg_color); _spr.drawString("SET", text_start_x, y + 6); }

        _spr.loadFont(Arial18); _spr.setTextDatum(TL_DATUM);
        if (isEditingThis) {
    ...
    _spr.setTextDatum(MC_DATUM); _spr.loadFont(Arial18); const char* status_text = "---"; uint16_t status_color = C_GREY_TXT;
    if (isOtherModeRunning) { status_text = "In-use"; status_color = C_GREY_TXT; }
    else if (activePresetIdx >= 0) {
        if (isLocked) { status_text = "Lock"; status_color = _blink_state ? C_RED_TXT : C_BLACK; } else if (isReady) { status_text = "Ready"; status_color = C_GREEN_TXT; } else { status_text = "Heating"; status_color = TFT_ORANGE; }
    } else if (_manual_confirmed_preset >= 0) { status_text = "Standby"; status_color = TFT_BLUE; }
    _spr.setTextColor(TFT_WHITE, C_BLACK); snprintf(buf, 40, "Status : "); _spr.setTextDatum(MR_DATUM); _spr.drawString(buf, w/2, status_line_y + status_line_h/2);
