  ...
  for (int i = 0; i < 4; i++) {
    if (btn_current[i] && !pcf_state.btn_pressed[i]) {
      if (now - pcf_state.last_press[i] > pcf_state.DEBOUNCE_MS) {
        ...
        switch(i) {
          case 0: config.heater_active[0] = !config.heater_active[0]; saveConfig(config); break;
...
void setup() {
  ...
  tft.init();
  tft.setRotation(3); // 180 Deg Rotation
  tft.fillScreen(TFT_BLACK);
  esp_task_wdt_reset();

  ui.begin(); // Require PSRAM Enabled
  esp_task_wdt_reset();
...
void loop() {
  ...
  if (beeper_active && millis() >= t_next_beep_action) {
    if (beep_queue > 0) {
      ...
      beep_queue--;
    } else { digitalWrite(BUZZER, LOW); beeper_active = false; }
  }
  ...
  if (now - t_sens >= 100) { t_sens = now; heater_read(); read_mlx_sensors(); }
  if (now - t_ctl >= 50) { t_ctl = now; updateControl(); }
...
void read_hardware_encoder() {
  ...
  if (pcnt_get_counter_value(PCNT_UNIT_0, &val) == ESP_OK) {
    if (abs(val) > 30000) { pcnt_counter_clear(PCNT_UNIT_0); lastEncoderValue = 0; return; }
...
void check_encoder_switch() {
  ...
  if (lastSwitchState == HIGH && cur == LOW) {
    queue_beeps(1);
    if (click_state == IDLE) { first_click_time = millis(); click_state = AWAITING_SECOND_CLICK; }
    else { ui.handleButtonDoubleClick(config); click_state = IDLE; }
  }
  lastSwitchState = cur;
}
...
void heater_read() {
  for (int i = 0; i < NUM_THERMOCOUPLES; ++i) {
    float tc, tj; uint8_t fb; uint32_t raw;
    if (max31855_read_celsius(TC_CS_PINS[i], &tc, &tj, &fb, &raw)) tc_temp_display[i] = tc + config.tc_offsets[i];
    else tc_temp_display[i] = NAN;
    tc_fault_bits[i] = fb;
  }
}
...
void updateControl() {
  if (!has_go_to) { for (int i=0; i<3; i++) tpo_set_percent(i, 0); return; }
  for (int i = 0; i < NUM_THERMOCOUPLES; i++) {
    if (!config.heater_active[i] || isnan(tc_temp_display[i])) { tpo_set_percent(i, 0); continue; }
    if (tc_temp_display[i] >= config.max_temps[i]) {
      tpo_set_percent(i, 0); config.heater_active[i] = false;
      heater_cutoff_state[i] = true; heater_cutoff_start_time[i] = millis();
      continue;
    }
...
void saveConfig(const ConfigState& cfg) { preferences.putBytes("config", &cfg, sizeof(cfg)); }
void loadConfig(ConfigState& cfg) { if (preferences.getBytesLength("config") == sizeof(cfg)) preferences.getBytes("config", &cfg, sizeof(cfg)); }

void read_heater_input() {
  if (Serial.available()) {
    String cmd = Serial.readStringUntil('\n'); cmd.trim();
    if (cmd.startsWith("PWM")) { pwm_override_enabled = true; pwm_override_percent = cmd.substring(3).toFloat(); }
